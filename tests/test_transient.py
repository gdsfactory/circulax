...
@pytest.mark.parametrize("backend", _backends)
def test_bdf2_vectorized_runs_float(simple_lrc_netlist, backend):
    """BDF2VectorizedTransientSolver should produce a finite trajectory on a real LRC circuit."""
    net_dict, models_map = simple_lrc_netlist
    groups, sys_size, _ = compile_netlist(net_dict, models_map)
    ...
    solver = BDF2VectorizedTransientSolver(linear_solver=linear_strat)
    term = diffrax.ODETerm(lambda t, y, args: jnp.zeros_like(y))
...
@pytest.mark.parametrize("backend", _backends)
def test_bdf2_vectorized_runs_complex(simple_optical_netlist, backend):
    """BDF2VectorizedTransientSolver should produce a finite trajectory on a complex optical circuit."""
    net_dict, models_map = simple_optical_netlist
    groups, sys_size, _ = compile_netlist(net_dict, models_map)
    ...
    t_max = 1.0e-9
    saveat = diffrax.SaveAt(ts=jnp.linspace(0, t_max, 10))
    ...
    assert sol.ys.shape == (10, 2 * sys_size)
    assert jnp.isfinite(sol.ys).all()
